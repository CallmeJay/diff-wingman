...
const analysisLabel = { not_generated: '未分析', explained: '已解释', pending: '待确认', unreviewed: '未分析' };
...
export function V8ReviewPanel({ review, changeId, fresh, stale, busy, canGenerate, onGenerateHunk, onUnderstanding, onChange, onSource }: {
  review: SavedReview;
  ...
  return <section className="v8-panel">
    ...
    <details className="v8-matrix">
      <summary>查看全部变更与未分析项</summary>
      <div className="v8-matrix-table" role="table" aria-label="变更覆盖矩阵">
        <div role="row" className="v8-matrix-heading"><span>变更</span><span>需求</span><span>AI</span><span>人工</span><span>评论</span><span>验证</span></div>
        {rows.map((row) => <button type="button" role="row" className={row.change.id === active?.change.id ? 'selected' : ''}
          key={row.change.id} onClick={() => onChange(row.change.id)}>
          <span title={row.file.path}>{row.file.path} · {row.change.label}</span>
          <span>{row.requirementIds.length ? row.requirementIds.join(', ') : '未关联'}</span>
          <span>{analysisLabel[row.analysis]}</span><span>{shownStatus(row.understandingStatus)}</span>
          <span>{row.unresolvedComments}/{row.comments}{row.fileComments ? ` + 文件 ${row.fileComments}` : ''}</span>
          <span>{row.verifications.length}</span>
        </button>)}
      </div>
